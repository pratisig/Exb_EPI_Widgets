...
const defaultSource: SourceConfig = { dateField: '', boundaryField: '', valueField: '', valueType: 'number', statistic: 'count', period: 'epi-week', weekMode: 'iso', dateConvention: 'dmy', outbreakStart: '', filterMode: 'cumulative', scaleMode: 'dynamic', fillMissingPeriods: true, decimalPlaces: 2 }
...
export default function Setting(props: AllWidgetSettingProps<IMConfig>) {
  ...
  // The aggregation engine already reads text weeks (EpiWeek), so text fields can be
  // exposed in the date selector on demand instead of being hidden permanently.
  const [textDateFields, setTextDateFields] = React.useState<{ [id: string]: boolean }>({})
  React.useEffect(() => {
    let active = true
  ...
  const updateField = (source: any, key: 'dateField' | 'valueField', field: string, valueType?: 'number' | 'date' | 'text') => {
    ...
    if (key === 'dateField') nextConfig = nextConfig.setIn(['sources', source.dataSourceId, 'dateFieldType'], valueType === 'text' ? 'text' : 'date')
  ...
  return <div className="epi-setting p-3">
    ...
    <Label>Analyse temporelle</Label><Select value={config.get?.('comparisonMode') || 'continuous'} onChange={e => update('comparisonMode', e.target.value)}><Option value="continuous">Série continue</Option><Option value="compare-years">Comparer les années</Option></Select>
    {config.get?.('comparisonMode') === 'compare-years' && <><Label>Années à comparer (séparées par des virgules)</Label><TextInput value={(config.get?.('comparisonYears') || []).join(', ')} placeholder="2023, 2024, 2025" onChange={e => update('comparisonYears', e.target.value.split(',').map(value => Number(value.trim())).filter(value => isFinite(value)))} /><Label>Année de référence</Label><TextInput type="number" value={String(config.get?.('referenceYear') || '')} onChange={e => update('referenceYear', Number(e.target.value) || undefined)} /></>}
    <Label>Couche frontière / géométrie (optionnel)</Label><Select value={config.get?.('boundarySourceId') || ''} onChange={e => update('boundarySourceId', e.target.value)}><Option value="">Utiliser la géométrie de chaque source</Option>{selectedSources.map(source => <Option key={source.dataSourceId} value={source.dataSourceId}>{source.dataSourceId}</Option>)}</Select>
    ...
    {selectedSources.map((source, index) => {
      ...
      return <div className="epi-source-setting" key={id}><h4>Source {index + 1}</h4><TextInput value={c.label || ''} placeholder="Nom court de la source (optionnel)" onChange={e => updateSource(id, 'label', e.target.value)} />
        <Label>Champ de regroupement spatial (optionnel)</Label><Select value={c.boundaryField || ''} onChange={e => updateSource(id, 'boundaryField', e.target.value)}><Option value="">Aucun regroupement par zone</Option>{textFields.map(field => <Option key={field.name} value={field.name}>{field.alias || field.name}</Option>)}</Select>
        <Label>Champ date ou semaine</Label><Select value={c.dateField || ''} onChange={e => { const selected = allFields.find(field => field.name === e.target.value); updateField(source, 'dateField', e.target.value, isDate(selected) ? 'date' : (selected ? 'text' : undefined)) }}><Option value="">{!schema ? 'Chargement du schéma...' : ((dateFields.length || (textDateFields[id] && textFields.length)) ? 'Sélectionner le champ temporel' : 'Aucun champ Date détecté')}</Option>{dateFields.map(field => <Option key={field.name} value={field.name}>{field.alias || field.name} (Date)</Option>)}{textDateFields[id] && textFields.map(field => <Option key={field.name} value={field.name}>{field.alias || field.name} (Texte)</Option>)}</Select>
        <label className="epi-toggle"><input type="checkbox" checked={!!textDateFields[id]} onChange={e => setTextDateFields(previous => ({ ...previous, [id]: e.target.checked }))} /><span>Afficher aussi les champs texte (semaines EpiWeek : <code>2025-W04</code>, <code>Week 03-2025</code>, <code>S01_2026</code>)</span></label>
        {c.dateFieldType === 'text' && <div className="epi-setting-warning">Champ texte sélectionné : l'agrégation, la timeline et les indicateurs fonctionnent, mais le filtrage de la source, de la carte et des composants connectés nécessite un champ Date natif.</div>}
