  ...
  return <div className="epi-setting p-3">
    ...
    {selectedSources.map((source, index) => {
      ...
      return <div className="epi-source-setting" key={id}><h4>Source {index + 1}</h4><TextInput value={c.label || ''} placeholder="Nom court de la source (optionnel)" onChange={e => updateSource(id, 'label', e.target.value)} />
        ...
        <Label>Type de mesure</Label><Select value={c.metricMode || 'aggregate'} onChange={e => updateSource(id, 'metricMode', e.target.value)}><Option value="aggregate">Statistique d'un champ</Option><Option value="rate">Taux (numérateur / dénominateur)</Option></Select>
        {c.metricMode === 'rate' && <><Label>Numérateur</Label><Select value={c.numeratorField || ''} onChange={e => updateSource(id, 'numeratorField', e.target.value)}><Option value="">Sélectionner le numérateur</Option>{numericFields.map(field => <Option key={field.name} value={field.name}>{field.alias || field.name}</Option>)}</Select><Label>Dénominateur</Label><Select value={c.denominatorField || ''} onChange={e => updateSource(id, 'denominatorField', e.target.value)}><Option value="">Sélectionner le dénominateur</Option>{numericFields.map(field => <Option key={field.name} value={field.name}>{field.alias || field.name}</Option>)}</Select><Label>Facteur du taux</Label><TextInput type="number" value={String(c.rateFactor ?? 100)} onChange={e => updateSource(id, 'rateFactor', Number(e.target.value) || 1)} /></>}
