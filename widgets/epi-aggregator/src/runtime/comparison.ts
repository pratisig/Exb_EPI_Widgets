...
import { aggregate, parseEpiDate } from './aggregation'
...
export function compareYears(records: any[], dateField: string, period: Period, years: number[], weekMode: WeekMode, statistic: Statistic, valueField?: string, valueType: 'number' | 'date' | 'text' = 'number', convention: DateConvention = 'dmy', outbreakStart?: string, referenceYear?: number): ComparisonRow[] {
  ...
  selected.forEach(year => {
    ...
    result.rows.forEach(row => { const comparisonKey = period === 'year' ? 'year' : period === 'month' ? row.key.slice(5) : period === 'quarter' ? row.key.slice(5) : row.key.slice(row.key.indexOf('-W') + 2); const existing = groups.get(comparisonKey) || { periodKey: comparisonKey, label: period === 'epi-week' ? `S${comparisonKey}` : row.label, values: {} }; existing.values[year] = Number(row.value) || 0; groups.set(comparisonKey, existing) })
  })
