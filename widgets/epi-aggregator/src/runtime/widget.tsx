...
interface ViewProps { ds: DataSource; props: AllWidgetProps<IMConfig>; fr: boolean; field: string; period: Period; weekMode: WeekMode; outbreak: string; setField: (v: string) => void; setPeriod: (v: Period) => void; setWeekMode: (v: WeekMode) => void; setOutbreak: (v: string) => void }

function EpiView({ ds, props, fr, field, period, weekMode, outbreak, setField, setPeriod, setWeekMode, setOutbreak }: ViewProps) {
  ...
  React.useEffect(() => {
    ...
    ds.query({ where: '1=1', outFields: ['*'], returnGeometry: false, pageSize: 10000 } as any).then((result: any) => {
      if (cancelled) return
      const output = aggregate(result.records || [], field, period, weekMode, outbreak)
  ...
  }, [ds, field, period, weekMode, outbreak])
  React.useEffect(() => {
  ...
  return <div className="epi-widget">
    ...
    <div className="epi-toolbar"><Button size="sm" onClick={() => setPlaying(!playing)} disabled={!rows.length}>{playing ? '■' : '▶'} {playing ? t(fr, 'Stop', 'Arrêter') : t(fr, 'Play', 'Lire')}</Button><span>{t(fr, 'Select a period to filter connected widgets', 'Sélectionnez une période pour filtrer les widgets connectés')}</span></div>
    <div className="epi-periods">{rows.map(r => <button className={selected === r.key ? 'selected' : ''} key={r.key} onClick={() => { setSelected(r.key); setPlaying(false) }}><strong>{r.label}</strong><span>{r.count.toLocaleString(fr ? 'fr-FR' : 'en-US')}</span></button>)}</div>
    <div className="epi-foot">{selected ? `${t(fr, 'Active filter', 'Filtre actif')} : ${rows.find(r => r.key === selected)?.label}` : t(fr, 'No period selected', 'Aucune période sélectionnée')}</div>
  </div>
}
...
export default function Widget(props: AllWidgetProps<IMConfig>) {
  ...
  return <DataSourceComponent useDataSource={useDS} widgetId={props.id}>{(ds: DataSource) => <EpiView ds={ds} props={props} fr={fr} field={field} period={period} weekMode={weekMode} outbreak={outbreak} setField={setField} setPeriod={setPeriod} setWeekMode={setWeekMode} setOutbreak={setOutbreak} />}</DataSourceComponent>
}
