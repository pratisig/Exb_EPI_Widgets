...
import { IMConfig, Period, WeekMode, DateConvention, Statistic } from '../config'
import { aggregate, AggregateRow } from './aggregation'
import './style.css'

const t = (fr: boolean, en: string, f: string) => fr ? f : en

interface ViewProps { ds: DataSource; props: AllWidgetProps<IMConfig>; fr: boolean; field: string; valueField: string; statistic: Statistic; period: Period; weekMode: WeekMode; outbreak: string; convention: DateConvention; setField: (v: string) => void; setValueField: (v: string) => void; setStatistic: (v: Statistic) => void; setPeriod: (v: Period) => void; setWeekMode: (v: WeekMode) => void; setOutbreak: (v: string) => void; setConvention: (v: DateConvention) => void }

function EpiView({ ds, props, fr, field, valueField, statistic, period, weekMode, outbreak, convention, setField, setValueField, setStatistic, setPeriod, setWeekMode, setOutbreak, setConvention }: ViewProps) {
  ...
  React.useEffect(() => {
    ...
    ds.query({ where: '1=1', outFields: ['*'], returnGeometry: false, pageSize: 10000 } as any).then((result: any) => {
      if (cancelled) return
      const output = aggregate(result.records || [], field, period, weekMode, outbreak, convention, statistic, valueField)
  ...
  }, [ds, field, valueField, statistic, period, weekMode, outbreak, convention])
  React.useEffect(() => {
  ...
  const metricTotal = rows.reduce((n, r) => n + r.value, 0)
  const formatValue = (v: number) => Number.isInteger(v) ? v.toLocaleString(fr ? 'fr-FR' : 'en-US') : v.toLocaleString(fr ? 'fr-FR' : 'en-US', { maximumFractionDigits: 2 })
  return <div className="epi-widget">
    ...
    <div className="epi-controls">
      ...
      <Label>{t(fr, 'Statistic', 'Statistique')}</Label><Select value={statistic} onChange={e => setStatistic(e.target.value as Statistic)}><Option value="count">{t(fr, 'Count records', 'Compter les enregistrements')}</Option><Option value="sum">{t(fr, 'Sum', 'Somme')}</Option><Option value="mean">{t(fr, 'Mean / average', 'Moyenne')}</Option><Option value="median">{t(fr, 'Median', 'Médiane')}</Option><Option value="min">{t(fr, 'Minimum', 'Minimum')}</Option><Option value="max">{t(fr, 'Maximum', 'Maximum')}</Option><Option value="first">{t(fr, 'First value', 'Première valeur')}</Option><Option value="last">{t(fr, 'Last value', 'Dernière valeur')}</Option><Option value="distinct">{t(fr, 'Distinct values', 'Valeurs distinctes')}</Option></Select>{statistic !== 'count' && <><Label>{t(fr, 'Numeric / measure field', 'Champ numérique / mesure')}</Label><TextInput value={valueField} placeholder={t(fr, 'e.g. deaths, population', 'ex. décès, population')} onChange={e => setValueField(e.target.value)} /></>}
    ...
    {rows.length > 0 && <div className="epi-summary"><span><b>{formatValue(statistic === 'count' ? records : metricTotal)}</b><small>{statistic === 'count' ? t(fr, 'valid records', 'cas valides') : t(fr, 'total measure', 'mesure totale')}</small></span><span><b>{rows.reduce((m, r) => Math.max(m, r.count), 0).toLocaleString(fr ? 'fr-FR' : 'en-US')}</b><small>{t(fr, 'peak in a period', 'pic sur une période')}</small></span><span><b>{rows.length}</b><small>{t(fr, 'periods', 'périodes')}</small></span></div>}
    ...
    <div className="epi-periods">{rows.map((r, i) => <button className={selected === r.key ? 'selected' : ''} key={r.key} onClick={() => { setSelected(r.key); setPlaying(false) }}><strong>{r.label}</strong><span>{formatValue(cumulative ? rows.slice(0, i + 1).reduce((n, x) => n + x.value, 0) : r.value)}</span></button>)}</div>
...
export default function Widget(props: AllWidgetProps<IMConfig>) {
  ...
  const [valueField, setValueField] = React.useState(cfg.valueField || '')
  const [statistic, setStatistic] = React.useState<Statistic>(cfg.statistic || 'count')
  ...
  return <DataSourceComponent useDataSource={useDS} widgetId={props.id}>{(ds: DataSource) => <EpiView ds={ds} props={props} fr={fr} field={field} valueField={valueField} statistic={statistic} period={period} weekMode={weekMode} outbreak={outbreak} convention={convention} setField={setField} setValueField={setValueField} setStatistic={setStatistic} setPeriod={setPeriod} setWeekMode={setWeekMode} setOutbreak={setOutbreak} setConvention={setConvention} />}</DataSourceComponent>
}
